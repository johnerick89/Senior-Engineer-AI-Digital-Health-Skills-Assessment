...
def make_json_safe(value: Any) -> Any:
    """Convert common values into JSON-safe payloads for structured logs."""
...
def log_event(logger: Any, event_name: str, **payload: Any) -> None:
    """Emit a structured event payload that is safe for JSON logging."""
    sanitized = {
        key: make_json_safe(value)
        for key, value in payload.items()
        if value is not None and key != "event"
    }
    logger.info(event_name, **sanitized)
